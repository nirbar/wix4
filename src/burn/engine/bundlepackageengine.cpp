    ...
    )
{
    ...
    for (DWORD i = 0; i < cElements; ++i)
    {
        ...
        else if (CSTR_EQUAL == ::CompareStringOrdinal(sczAction, -1, L"Uninstall", -1, FALSE))
        {
    ...
    )
{
    ...
    hr = ExecuteBundle(pCache, pVariables, fRollback, TRUE, pfnGenericMessageHandler, pvContext, action, relationType, pPackage, TRUE, fPerMachine, wzParent, wzIgnoreDependencies, wzAncestors, wzEngineWorkingDirectory, pRestart);
    ...
    )
{
    ...
    if (fRunEmbedded)
    {
        ...
        if (pPackage->Bundle.fWin64 && pPackage->Bundle.pEngineVersion)
        {
            if (pPackage->Bundle.pEngineVersion->dwRevision) // psw-wix has revision, official WiX doesn't
            {
                VerCompareIsInRange(pPackage->Bundle.pEngineVersion, L"4.0.0.0", L"5.0.0.51", TRUE, TRUE, FALSE, &fPipeStrLen64);
            }
            else
            {
