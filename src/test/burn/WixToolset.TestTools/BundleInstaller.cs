...
namespace WixToolset.TestTools
{
    ...
    public partial class BundleInstaller : IDisposable
    {
        ...
        private string RunBundleWithArguments(int expectedExitCode, MSIExec.MSIExecMode mode, string[] arguments, bool assertOnError = true, string bundlePath = null, string layoutDirectory = null)
        {
            ...
            string logBaseName = String.Format("{0}_{1}_{2:yyyyMMddhhmmss}_{4}_{3}", this.TestGroupName, this.TestName, DateTime.UtcNow, Path.GetFileNameWithoutExtension(this.Bundle), mode);
            string logPattern = logBaseName + "*.log";
            string logFile = Path.Combine(this.LogDirectory, logBaseName + ".log");
            ...
            this.LogFiles.AddRange(Directory.GetFiles(this.LogDirectory, logPattern, SearchOption.TopDirectoryOnly));
            this.LogFiles.Sort((a,b) => a.Length - b.Length); // Main log first, packages later, sub packages later...
